...
"""Turn one DiffusionDrive prediction into Drive RPC trajectories.
...
class CachedPlan:
    """One DiffusionDrive prediction expressed in the rollout's local frame.
...
def make_cached_plan(
    ...
    """Convert an eight-step DiffusionDrive prediction into a cached plan."""
    poses = np.asarray(relative_poses, dtype=np.float64)
    if poses.shape != (8, 3):
        raise ValueError(
            f"DiffusionDrive prediction must have shape (8, 3); got {poses.shape}"
        )
