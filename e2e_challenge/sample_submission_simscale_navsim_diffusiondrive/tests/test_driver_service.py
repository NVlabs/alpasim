...
def _prediction() -> np.ndarray:
    return np.column_stack([np.arange(1, 9), np.zeros(8), np.zeros(8)]).astype(
        np.float32
    )


def _service(worker: FakeWorker | None = None) -> NavsimDiffusionDriveDriver:
    if worker is None:
        worker = FakeWorker(_prediction())
    return NavsimDiffusionDriveDriver(FakePolicyHandle(worker))
...
def _submit_images(
    service: NavsimDiffusionDriveDriver,
    session_uuid: str = "session",
...
def test_start_session_preserves_noise_state() -> None:
    service = _service()

    service.start_session(_session_request(random_seed=1234), FakeContext())

    session = service._sessions["session"]
    assert session.random_seed == 1234
    assert session.noise_index == 0
...
def test_repeated_frame_uses_cache_and_new_frame_runs_inference() -> None:
    ...
    assert service._sessions["session"].counters.diffusiondrive_inference == 2
...
def test_close_session_does_not_restore_inflight_state() -> None:
    ...
    assert pose_at_half_second.pose.vec.x == pytest.approx(1.0)
...
def test_same_session_concurrent_drive_reserves_frame_once() -> None:
    ...
    assert service._sessions["session"].counters.diffusiondrive_inference == 1
...
def test_base_exception_clears_reservation_and_propagates() -> None:
    ...
    assert len(worker.calls) == 2
    assert session.counters.diffusiondrive_inference == 1
    assert session.counters.inference_error == 0
...
def test_worker_unavailable_does_not_consume_frame() -> None:
    ...
    service = NavsimDiffusionDriveDriver(handle)
    service.start_session(_session_request(), FakeContext())
    _submit_egomotion(service)
    _submit_images(service)
...
def test_policy_handle_start_is_idempotent_and_stop_prevents_restart() -> None:
    ...
    assert entered.wait(timeout=1.0)
    assert handle._thread is not None
    assert handle._thread.name == "diffusiondrive-policy-loader"
...
def test_get_version_reports_ready_policy_and_git_hash(
    ...
    service = NavsimDiffusionDriveDriver(handle)
    monkeypatch.setenv("DIFFUSIONDRIVE_GIT_HASH", "test-hash")

    version = service.get_version(common_pb2.Empty(), FakeContext(None))

    assert version.version_id == "simscale-diffusiondrive-navhard-e2e"
...
def test_runtime_log_labels_are_diffusiondrive_specific() -> None:
    source = Path(driver_module.__file__).read_text()
    assert "DiffusionDrive inference failed" in source
    assert "SimScale DiffusionDrive driver listening" in source
    assert "LTF inference failed" not in source
    assert "SimScale LTF driver listening" not in source


def test_driver_propagates_session_seed_and_inference_index() -> None:
    worker = FakeWorker(_prediction())
    service = _service(worker)
    service.start_session(_session_request(random_seed=1234), FakeContext())
    _submit_egomotion(service)
    ...
    _drive(service)
    _submit_images(service, timestamp_us=1_500_000)
    _drive(service, time_now_us=1_500_000)

    assert [request.noise_seed for request in worker.calls] == [1234, 1234]
    assert [request.noise_index for request in worker.calls] == [0, 1]
...
def _install_main_fakes(
    ...
    *,
    ...
    monkeypatch.setattr(
        ...
        lambda checkpoint_path, device: (checkpoint_path, device),
    )
...
def test_main_wires_server_policy_and_environment(
    ...
    monkeypatch.setenv("DIFFUSIONDRIVE_CHECKPOINT_PATH", "/tmp/test.ckpt")
    monkeypatch.setenv("DIFFUSIONDRIVE_DEVICE", "cpu")
    monkeypatch.setenv("DIFFUSIONDRIVE_MAX_BATCH_SIZE", "3")
    monkeypatch.setenv("DIFFUSIONDRIVE_BATCH_WINDOW_MS", "7.5")
    ...
    assert loader() == ("/tmp/test.ckpt", "cpu")
    service = captures["service"]
    assert isinstance(service, NavsimDiffusionDriveDriver)
