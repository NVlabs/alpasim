...
NOW_US = 1_000_000
QUERY_US = 1_500_000
TRAJECTORY_END_US = 5_000_000
VERSION_ID = os.environ.get(
    "GTRS_PROBE_VERSION_ID",
...
def trajectory_summary(
    response: egodriver_pb2.DriveResponse,
    session_uuid: str,
    expected_x: float,
) -> dict[str, int | str]:
    poses = list(response.trajectory.poses)
    timestamps = [pose.timestamp_us for pose in poses]
    _require(bool(timestamps), "drive response trajectory is empty")
    _require(
        timestamps[0] == NOW_US,
    ...
    _require(
        len(timestamps) == 41,
        "trajectory must contain exactly 41 points",
    )
    _require(
        timestamps[-1] == TRAJECTORY_END_US,
        "trajectory must end at 5000000us",
    )
    _require(
    ...
    _require(
        ...
        "trajectory contains a non-finite pose",
    )
    _require(
        abs(poses[0].pose.vec.x - expected_x) < 1e-4,
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Probe a live SimScale GTRS-Dense driver"
    )
...
def main() -> None:
    ...
    try:
        ...
        def drive(session: tuple[str, float, int, float]) -> dict[str, int | str]:
            ...
            return trajectory_summary(response, session_uuid, expected_x)
