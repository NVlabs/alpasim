...
import navsim_gtrs_dense_challenge.batch_worker as batch_worker_module
import numpy as np
import pytest
from navsim_gtrs_dense_challenge.batch_worker import BatchWorker
...
def _prediction(marker: float) -> Prediction:
    return Prediction(
        trajectory=np.full((40, 3), marker, dtype=np.float32),
    )
...
def test_start_and_stop_are_idempotent_and_leave_no_thread() -> None:
    ...
    assert thread.name == "gtrs-batch-worker"
