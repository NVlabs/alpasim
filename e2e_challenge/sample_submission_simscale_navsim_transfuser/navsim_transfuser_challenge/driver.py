...
from .policy import InferenceInput, LtfPolicy, Prediction
...
class PolicyHandle:
    ...
    def _load(self) -> None:
        try:
            ...
            LOGGER.exception("LTF policy load failed")
        finally:
            self._ready.set()
...
class NavsimTransfuserDriver(egodriver_pb2_grpc.EgodriverServiceServicer):
    ...
    def drive(
        ...
        if reserved_timestamp_us is not None:
            ...
            try:
                try:
                    ...
                    inference_error = exc
                    LOGGER.exception(
                        "LTF inference failed for session %s timestamp_us=%d",
        ...
        with self._lock:
            ...
            if active_session is session:
                ...
                if fresh_plan is not None:
                    ...
                    session.counters.ltf_inference += 1
                ...
                counter_values = (
                    session.counters.ltf_inference,
        ...
        if counter_values is not None:
            LOGGER.info(
                "session=%s ltf_inference=%d cached_plan=%d "
    ...
    def get_version(
        ...
        if not self._policy_handle.wait_ready(timeout):
            context.abort(
                ...
                "LTF policy readiness timed out",
            )
            raise AssertionError("unreachable")
        ...
        if load_error is not None:
            context.abort(
                grpc.StatusCode.UNAVAILABLE,
                f"LTF policy load failed: {load_error}",
            )
            raise AssertionError("unreachable")
        if self._policy_handle.worker() is None:
            context.abort(
                grpc.StatusCode.UNAVAILABLE,
                "LTF policy reported ready without a batch worker",
            )
            raise AssertionError("unreachable")

        return common_pb2.VersionId(
            version_id="simscale-ltf-navtest-e2e",
            git_hash=os.environ.get("LTF_GIT_HASH", "local"),
...
def main() -> None:
    ...
    checkpoint_path = os.environ.get(
        "LTF_CHECKPOINT_PATH",
        "/app/assets/ltf/ltf_sim_navtest.ckpt",
    )
    device = os.environ.get("LTF_DEVICE", "cuda")
    max_batch_size = int(os.environ.get("LTF_MAX_BATCH_SIZE", "2"))
    batch_window_s = float(os.environ.get("LTF_BATCH_WINDOW_MS", "2")) / 1000.0

    policy_handle = PolicyHandle(
        loader=lambda: LtfPolicy(checkpoint_path, device=device),
        max_batch_size=max_batch_size,
        batch_window_s=batch_window_s,
    )
    service = NavsimTransfuserDriver(policy_handle)
    grpc_workers = max(
        1,
    ...
    try:
        server.start()
        LOGGER.info(
            "SimScale LTF driver listening on %s:%d",
            host,
            bound_port,
        )
        policy_handle.start()
