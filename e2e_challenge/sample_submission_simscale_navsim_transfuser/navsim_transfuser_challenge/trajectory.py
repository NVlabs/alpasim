...
"""Turn one LTF prediction into the trajectories the Drive RPC returns.
...
class CachedPlan:
    """One LTF prediction expressed in the rollout's local frame.
...
def make_cached_plan(
    ...
    """Convert an eight-step LTF prediction into a local-frame cached plan."""
    poses = np.asarray(relative_poses, dtype=np.float64)
    if poses.shape != (8, 3):
        raise ValueError(f"LTF prediction must have shape (8, 3); got {poses.shape}")

    times_s = np.arange(9, dtype=np.float64) * 0.5
