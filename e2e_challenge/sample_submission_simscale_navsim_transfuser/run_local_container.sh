...
PREFIX="${ALPASIM_DRIVER_NAME_PREFIX:-alpasim-e2e-simscale-ltf}"
...
docker_args() {
  ...
  args=(
    ...
    -e "LTF_DEVICE=${LTF_DEVICE:-cuda}"
    -e "LTF_MAX_BATCH_SIZE=${LTF_MAX_BATCH_SIZE:-2}"
    -e "LTF_BATCH_WINDOW_MS=${LTF_BATCH_WINDOW_MS:-2}"
  )
  if [[ "${detached}" == "1" ]]; then
    args+=(--detach)
  fi
