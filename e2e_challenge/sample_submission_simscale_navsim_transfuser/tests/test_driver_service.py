...
import navsim_transfuser_challenge.driver as driver_module
import numpy as np
import pytest
from alpasim_grpc import API_VERSION_MESSAGE
from alpasim_grpc.v0 import common_pb2, egodriver_pb2, sensorsim_pb2
from navsim_transfuser_challenge.driver import (
    NavsimTransfuserDriver,
    PolicyHandle,
    SessionCounters,
    SessionState,
    _context_timeout,
...
from navsim_transfuser_challenge.navigation import DriveCommand, command_one_hot
...
def _service(worker: FakeWorker | None = None) -> NavsimTransfuserDriver:
    if worker is None:
        worker = FakeWorker(_prediction())
    return NavsimTransfuserDriver(FakePolicyHandle(worker))
...
def _submit_images(
    service: NavsimTransfuserDriver,
    session_uuid: str = "session",
...
def test_repeated_frame_uses_cache_and_new_frame_runs_inference() -> None:
    ...
    assert service._sessions["session"].counters.ltf_inference == 2
...
def test_same_session_concurrent_drive_reserves_frame_once() -> None:
    ...
    assert service._sessions["session"].counters.ltf_inference == 1
...
def test_base_exception_clears_reservation_and_propagates() -> None:
    ...
    assert len(worker.calls) == 2
    assert session.counters.ltf_inference == 1
    assert session.counters.inference_error == 0
...
def test_worker_unavailable_does_not_consume_frame() -> None:
    ...
    service = NavsimTransfuserDriver(handle)
    service.start_session(_session_request(), FakeContext())
    _submit_egomotion(service)
    _submit_images(service)
...
def test_policy_handle_start_is_idempotent_and_stop_prevents_restart() -> None:
    ...
    assert entered.wait(timeout=1.0)
    assert handle._thread is not None
    assert handle._thread.name == "ltf-policy-loader"
...
def test_get_version_reports_ready_policy_and_git_hash(
    ...
    service = NavsimTransfuserDriver(handle)
    monkeypatch.setenv("LTF_GIT_HASH", "test-hash")

    version = service.get_version(common_pb2.Empty(), FakeContext(None))

    assert version.version_id == "simscale-ltf-navtest-e2e"
...
def test_main_wires_server_policy_and_environment(
    ...
    monkeypatch.setenv("LTF_CHECKPOINT_PATH", "/tmp/test.ckpt")
    monkeypatch.setenv("LTF_DEVICE", "cpu")
    monkeypatch.setenv("LTF_MAX_BATCH_SIZE", "3")
    monkeypatch.setenv("LTF_BATCH_WINDOW_MS", "7.5")
    ...
    assert isinstance(service, NavsimTransfuserDriver)
