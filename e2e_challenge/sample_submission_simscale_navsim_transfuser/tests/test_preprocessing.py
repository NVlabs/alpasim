...
import pytest
import torch
from navsim_transfuser_challenge.preprocessing import (
...
def test_preprocess_images_crops_stitches_resizes_and_normalizes() -> None:
    ...
    assert actual[0, 0, 256].item() == pytest.approx(32 / 255, abs=1 / 255)
    assert actual[0, -1, 1792].item() == pytest.approx(224 / 255, abs=1 / 255)
    assert actual[0, 256, 0].item() == pytest.approx(32 / 255, abs=1 / 255)
    assert actual[0, 256, -1].item() == pytest.approx(224 / 255, abs=1 / 255)
...
def test_build_status_feature_concatenates_float32_vectors() -> None:
    ...
    expected = torch.tensor([0, 1, 0, 0, 4, -0.5, 0.2, 0.1], dtype=torch.float32)
