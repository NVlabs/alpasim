...
def test_probe_main_success_emits_json_and_cleans_up(
                ...
                trajectory=probe_container.common_pb2.Trajectory(
                    poses=[
                        probe_container.common_pb2.PoseAtTime(
                            timestamp_us=probe_container.NOW_US,
                            pose=probe_container.common_pb2.Pose(
                                vec=probe_container.common_pb2.Vec3(x=expected_x)
                            ),
                        ),
                        probe_container.common_pb2.PoseAtTime(
                            timestamp_us=probe_container.QUERY_US,
                            pose=probe_container.common_pb2.Pose(
                                vec=probe_container.common_pb2.Vec3(x=expected_x + 2.5)
                            ),
                        ),
                    ]
                )
    ...
    assert output == {
        "sessions": [
            {"last_us": probe_container.QUERY_US, "points": 2, "session": "probe-a"},
            {"last_us": probe_container.QUERY_US, "points": 2, "session": "probe-b"},
        ],
