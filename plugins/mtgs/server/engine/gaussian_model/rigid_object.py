...
from ..utils.gaussian_utils import IDFT, interpolate_quats, quat_mult, quat_to_rotmat
...
class RigidPortableSubModel(VanillaPortableGaussianModel):
    """Rigid body Gaussian Splatting model with log-based motion."""
    ...
    def load_state_dict(self, dict: dict):
        ...
        if self.exist_log:
            ...
            if not self.static_in_log:
                ...
                self.log_timestamps = (
                    self.config.log_timestamps.squeeze() - self.log_start_time
                )

            log_type = "static" if self.static_in_log else "dynamic"
            logger.debug(
                f"Log pose for `{self.model_name_abbr}` loaded. LOG TYPE: `{log_type}`"
            )
            if self.log_replay:
    ...
    def get_fourier_features(self, x):
        ...
        idft_base = IDFT(
            scaled_x, self.config.fourier_features_dim, input_is_normalized
        ).to(self.device)
    ...
    def get_true_features_dc(self, timestamp=None, cam_obj_yaw=None):
        ...
        normalized_x = (
            timestamp if self.config.fourier_in_space == "temporal" else cam_obj_yaw
        )
    ...
    def get_gaussian_rgbs(self, camera_to_worlds, timestamp, device=None):
        ...
        assert device != torch.device(
            "cpu"
        ), "`sphereical_harmonics` in `gsplat` only supports CUDA"
        true_features_dc = self.get_true_features_dc(timestamp, None)
        colors = torch.cat(
            (true_features_dc[:, None, :], self.features_rest), dim=1
        ).to(device)
        if self.sh_degree > 0:
            viewdirs = self.global_means.detach().to(device) - camera_to_worlds[
                ..., :3, 3
            ].to(device)
    ...
    def _get_log_pose_from_timestamp(self, timestamp):
        ...
        quat_interp = interpolate_quats(
            self.log_quats[prev_frame], self.log_quats[next_frame], t
        ).squeeze()
        trans_interp = torch.lerp(
            self.log_trans[prev_frame], self.log_trans[next_frame], t
        )
