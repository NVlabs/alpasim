...
class VanillaPortableGaussianModel(torch.nn.Module):
    ...
    def get_gaussian_rgbs(self, camera_to_worlds, device=None, **kwargs):
        ...
        assert self.features_dc.dim() == 2, (
            "if Fourier embedding is used in models derived from `VanillaPortableGaussianModel`, ",
            "a new `get_gaussian_rgbs` function must be implemented",
        )
        colors = torch.cat(
            (self.features_dc[:, None, :], self.features_rest), dim=1
        ).to(device)
        if self.sh_degree > 0:
            viewdirs = self.means.detach().to(device) - camera_to_worlds[..., :3, 3].to(
                device
            )
