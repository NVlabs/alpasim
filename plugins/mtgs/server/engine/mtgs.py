...
from .base_renderer import BaseRenderer, RenderState
from .gaussian_model.rigid_object import RigidPortableSubModel as RigidModel
from .gaussian_model.rigid_object_mirrored import (
    MirroredRigidPortableSubModel as MirroredModel,
)
from .gaussian_model.vanilla_gaussian_splatting import (
    VanillaPortableGaussianModel as VanillaModel,
)
from .utils.gaussian_utils import matrix_to_quaternion, quat_to_angle, quat_to_rotmat
from .utils.geometry_utils import Sim2
from .utils.portable_utils import AttrDict, convert_to_attribute_dict

logger = logging.getLogger(__name__)


def _normalize_asset_id(asset_id: str) -> str:
    """Strip trailing '-NNN' version suffix from an asset ID (e.g. 'scene-001' → 'scene')."""
    if len(asset_id) > 4 and asset_id[-4] == "-":
        return asset_id[:-4]
    return asset_id
...
class MTGS(BaseRenderer):
    ...
    def _init_background_color(self, background_color):
        if isinstance(background_color, str):
            ...
            for color in background_color:
                assert 0 <= color <= 255
            return torch.tensor(background_color, device=self.device).float() / 255.0
    ...
    def _prepare_metas(self):
        ...
        self.submodel_names = {}
        self.timestamp = None
        # Cache key for `update_world` -- includes the set of agents present, not
        # just the timestamp, so that requests with the same t but different
    ...
    def reset(self, current_scene_id, asset_id: Optional[str] = None, **kwargs):
        ...
        self.mtgs_agent2states = {}  # populated by calibrate_agent_state() after reset
        self._initialized = True
        logger.info(
            f"Renderer reset completed for scene {current_scene_id} (asset: {asset_id})"
        )

    def set_world_to_nre(self, world_to_nre: np.ndarray):
        self.world_to_nre = world_to_nre
        logger.info(
            f"Set world_to_nre transformation: translation={world_to_nre[:3, 3]}"
        )
    ...
    def calibrate_agent_state(self, ego2globals: Optional[torch.Tensor] = None):
        ...
        if ego2globals is not None:
            if isinstance(ego2globals, np.ndarray):
                ego2globals_tensor = torch.tensor(
                    ego2globals, dtype=torch.float64, device=self.device
                )
            else:
                ego2globals_tensor = ego2globals.to(
                    dtype=torch.float64, device=self.device
                )

            if ego2globals_tensor.ndim == 2:
                ego2globals_tensor = ego2globals_tensor.unsqueeze(0)
        ...
        for asset_token, model_name in self.submodel_names.items():
            ...
            mtgs_agent2states[asset_token] = {
                "translation": self.gaussian_models[model_name]
                .log_trans[in_frame_mask]
                .double(),
                "rotation": self.gaussian_models[model_name]
                .log_quats[in_frame_mask]
                .double(),
            }
    ...
    def set_asset(self, asset):
        ...
        if is_background:
            self.recon2global_translation = asset["background"]["config"][
                "recon2world_translation"
            ]
        ...
        for asset_name in asset.keys():
            ...
            self.gaussian_models[model_name] = asset_class(
                asset=sub_asset, model_name=model_name
            )
    ...
    def set_sensors(self, sensors, ego2global):
        if self.sensor_caches is not None:
            camera_to_egos, intrinsics, map_inverse_distorts, hw_dict = (
                self.sensor_caches
            )
        ...
        for idx, (cam_name, cam_info) in enumerate(sensors.items()):
            ...
            intrinsic_torch = (
                torch.from_numpy(new_intrinsic).float().to(self.device).squeeze()
            )
            sensor2egos.append(sensor2ego)
            intrinsics.append(intrinsic_torch)
            map_inverse_distorts.append(map_inverse_distort)

            assert (height is None) or (
                height == cam_info["height"]
            ), "inconsistent height among RGB cameras"
            assert (width is None) or (
                width == cam_info["width"]
            ), "inconsistent width among RGB cameras"
    ...
    def update_world(self, timestamp, agent_states):
        cache_key = (timestamp, frozenset(agent_states.keys()))
        if (
            self._world_cache_key == cache_key
            and hasattr(self, "collected_gaussians")
            and self.collected_gaussians
            and "means" in self.collected_gaussians
        ):
            return
        gs_dict = {
            "means": [],
        ...
        for asset_token in self.node_types.keys():
            ...
            if asset_token in agent_states.keys():
                quat, trans = self.get_agent_pose(
                    asset_token, agent_states[asset_token]
                )
            else:
            ...
            try:
                ...
                logger.error(
                    f"Error collecting gaussians for {asset_token}: {e}", exc_info=True
                )
                raise
        ...
        if not gs_dict["means"]:
            logger.error(
                (
                    f"No gaussians collected in update_world! node_types: {list(self.node_types.keys())}, ",
                    f"agent_states: {list(agent_states.keys())}, timestamp={timestamp}",
                )
            )
            raise RuntimeError(
                f"No gaussians collected - no models available for timestamp {timestamp}"
            )
    ...
    def update_gaussian_rgbs(self, camera_to_worlds):
        ...
        for asset_token in self.node_types.keys():
            ...
            for i in range(camera_to_worlds.shape[0]):
                rgbs.append(
                    gaussian_model.get_gaussian_rgbs(
        ...
        self.collected_gaussians["rgbs"] = torch.cat(rgb_list, dim=1)
    ...
    def render(self, render_state: RenderState):
        ...
        ego2global_raw = self.get_agent_pose(
            "ego", converted_agent_states["ego"], return_matrix=True
        )
        ...
        R = camera_to_worlds[:, :3, :3]
        T = camera_to_worlds[:, :3, 3:4]
        R_inv = R.transpose(1, 2)
        T_inv = -R_inv @ T
        viewmat = self.get_transform_matrix(rotation=R_inv, translation=T_inv).to(
            self.device
        )
        ...
        for cam_name, cam_idx in self.sensor_mapping.items():
            ...
            for image_key in outputs.keys():
                sensor_dict[cam_name][image_key] = cv2.remap(
                    ...
                    map_inverse_distorts[cam_idx][1],
                    cv2.INTER_LINEAR,
                )
        return_dict = {
            "cameras": sensor_dict,
        ...
        ego2global_nuplan[:3, 3] = (
            ego2global_nuplan[:3, 3] + self.recon2global_translation[:3]
        )

        meta_data_dict = {"ego2global": ego2global_nuplan, "render_state": render_state}
    ...
    def get_agent_pose(self, name, agent_state, return_matrix=False):
        ...
        agent_state_xy = torch.tensor(
            agent_state[:2], device=self.device, dtype=torch.float64
        )
        agent_state_heading = torch.tensor(
            agent_state[-1], device=self.device, dtype=torch.float64
        )

        nearsest_idx = torch.argmin(
            torch.norm(mtgs_agent_state["translation"][:, :2] - agent_state_xy, dim=-1)
        )
...
class MTGSAssetManager:
    ...
    def load_asset(self):
        ...
        background_asset_path = (
            self.asset_dir / "background" / f"{self.current_asset_id}.ckpt"
        )
        self.background_asset_dict = torch.load(
            background_asset_path, map_location=self.device, weights_only=False
        )
        logger.info(
            f"Loaded background asset dict with {len(self.background_asset_dict)} models"
        )
