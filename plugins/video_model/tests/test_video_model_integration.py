...
"""End-to-end integration tests for the alpasim-video-model plugin.

These tests exercise the plugin against:
...
guarantee that the plugin's wiring, contract, and data-extraction code paths
...
from alpasim_runtime.config import CameraDefinitionConfig
...
def _build_synthetic_usdz(
    ...
    *,
    ...
    """Write a minimal USDZ that exercises the plugin's USDZ-parsing surface.
...
def test_register_scene_cameras_uses_usdz_calibration_and_applies_overrides(
    tmp_path: Path,
) -> None:
    """USDZ supplies real calibration; local overrides patch resolution + shutter.
    ...
    assert (
        cam.intrinsics.shutter_type == sensorsim_pb2.ShutterType.ROLLING_TOP_TO_BOTTOM
    )
    # Extrinsics inherited from USDZ (not the rig origin):
...
def test_register_scene_cameras_drops_overrides_without_matching_usdz_camera(
    tmp_path: Path, caplog: pytest.LogCaptureFixture
) -> None:
    """An override with no matching USDZ camera is dropped with a warning.

    Without real intrinsics we can't meaningfully send the camera to the
    video model, so we log + skip rather than fall back to a synthesized one.
    """
    usdz = _build_synthetic_usdz(
        tmp_path / "scene.usdz",
        scene_id="clipgt-x",
    ...
    with caplog.at_level("WARNING", logger="alpasim_video_model.service"):
        service._register_scene_cameras(scene_id="clipgt-x", usdz_path=str(usdz))

    # Front camera survived, the bogus override was dropped (but still ends up
    # registered from the USDZ since it was present there).
    defs = catalog.get_camera_definitions("clipgt-x")
    assert "camera_front_wide_120fov" in defs
    assert "camera_does_not_exist" not in defs
    assert any(
        "camera_does_not_exist" in rec.message for rec in caplog.records
    ), "Expected a warning about the dropped override"


def test_apply_override_resolution_and_shutter() -> None:
    """``_apply_override`` honors only resolution_hw and shutter_type."""
    ...
    assert (
        out.intrinsics.shutter_type == sensorsim_pb2.ShutterType.ROLLING_TOP_TO_BOTTOM
    )
