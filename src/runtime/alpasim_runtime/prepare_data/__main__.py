...
def preprocess_basic(config: Any, verbose: bool = True) -> bool:
    ...
    Args:
        config: Configuration with a to_trajdata_params() method
                (supports PrepareDataConfig from CLI).
    ...
    return preprocess_from_params(params, verbose=verbose)


def preprocess_from_params(params: dict, verbose: bool = True) -> bool:
    """Build trajdata cache from pre-built params dict.

    Args:
        params: Dictionary with keys expected by UnifiedDataset constructor.
        verbose: Whether to show verbose output from trajdata.
...
def run_from_user_config(config_path: str, args: argparse.Namespace) -> bool:
    ...
    # Convert config to trajdata params and run preprocessing
    params = trajdata_provider_config_to_params(config)
    return preprocess_from_params(params, verbose=args.verbose)
