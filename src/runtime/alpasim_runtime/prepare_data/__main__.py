...
def preprocess_basic(config: Any, verbose: bool = True) -> bool:
    ...
    Args:
        config: Configuration (supports both PrepareDataConfig from CLI and
                TrajdataProviderConfig from user config).
        verbose: Whether to show verbose output from trajdata.

    Returns:
        True if successful, False otherwise.
    """
    if isinstance(config, PrepareDataConfig):
        params = config.to_trajdata_params()
    else:
        params = trajdata_provider_config_to_params(config)
    trajdata_desired_data = list(params["desired_data"])

    if not trajdata_desired_data:
        logger.info("No trajdata-backed sources requested; nothing to preprocess.")
        return True

    params["desired_data"] = trajdata_desired_data
    params["data_dirs"] = {
        dataset_name: params["data_dirs"][dataset_name]
        for dataset_name in trajdata_desired_data
    }
    ...
    try:
        dataset = UnifiedDataset(
            ...
            dataset_kwargs={
                ...
                if dataset_name in trajdata_desired_data
            },
            verbose=verbose,
        )
    except Exception as e:
...
def run_from_cli(args: argparse.Namespace) -> bool:
    ...
    if missing:
        logger.error(f"Missing data directories for requested data sources: {missing}")
        logger.error(
            "Provide --data-dir for each requested data source: dataset=/path or in order"
        )
        return False
