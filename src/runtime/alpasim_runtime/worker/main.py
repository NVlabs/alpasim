...
from alpasim_utils.services import DEFAULT_RENDERER_TYPE, normalize_renderer_type
...
async def run_single_rollout(
    ...
    try:
        ...
        # Resolve the renderer service: built-in SensorsimService for the
        # default path, or a plugin-provided service discovered via
        # ``alpasim.services``.
        renderer_type = normalize_renderer_type(user_config.renderer_type)
        if renderer_type == DEFAULT_RENDERER_TYPE:
            ...
            services_registry = PluginRegistry("alpasim.services")
            renderer_service_class = services_registry.get(renderer_type)
            renderer_service = renderer_service_class.from_config(
                raw_config=user_config.renderer_config or {},
