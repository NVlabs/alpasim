...
def test_batch_return_marks_failed_item_retryable():
    items = [
        _item("cam_a"),
    ...
    with pytest.raises(
        RetryableRendererInfrastructureError, match=r"cam_b.*actor editing disabled"
    ):
        SensorsimService._batch_return_to_images(items, _triggers())


@pytest.mark.asyncio
async def test_batch_render_marks_rpc_failure_retryable(monkeypatch):
    svc = SensorsimService("addr:0", skip=False, camera_catalog=MagicMock())
    svc.session_info = SessionInfo(
        uuid="session-1",
        broadcaster=SimpleNamespace(broadcast=AsyncMock()),
    )
    svc.stub = SimpleNamespace(batch_render_rgb=object())
    svc.get_available_ego_masks = AsyncMock(return_value=AvailableEgoMasksReturn())
    svc.determine_ego_mask_id = MagicMock(return_value=None)
    svc.construct_rgb_render_request = MagicMock(
        return_value=RGBRenderRequest(scene_id="scene")
    )

    async def _raise_rpc_failure(*args, **kwargs):
        del args, kwargs
        raise RuntimeError("future renderer failure")

    monkeypatch.setattr(
        "alpasim_runtime.services.sensorsim_service.profiled_rpc_call",
        _raise_rpc_failure,
    )

    cam = SimpleNamespace(logical_id="cam_a")
    with pytest.raises(
        RetryableRendererInfrastructureError, match="future renderer failure"
    ):
        await svc.batch_render(
            [(cam, _triggers()["cam_a"])],
