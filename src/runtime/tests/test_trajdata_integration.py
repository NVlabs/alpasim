...
def test_build_pending_jobs_with_valid_scene():
    ...
    jobs = build_pending_jobs_from_request(
        request, lambda scene_id: scene_id in {"scene_a", "scene_b"}
    )
...
def test_build_pending_jobs_with_unknown_scene():
    ...
    with pytest.raises(UnknownSceneError) as exc_info:
        build_pending_jobs_from_request(request, lambda _scene_id: False)
...
def test_build_pending_jobs_drops_zero_rollouts():
    ...
    jobs = build_pending_jobs_from_request(request, lambda _scene_id: True)
