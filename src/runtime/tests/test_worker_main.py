...
@pytest.mark.asyncio
async def test_run_single_rollout_uses_generic_renderer_service_handoff(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_renderer_service = object()
    captured: dict[str, object] = {}

    class FakeRendererServiceClass:
        @classmethod
        def from_config(cls, raw_config, address, skip=False, *, camera_catalog=None):
    ...
    class FakeRegistry:
        ...
        def get(self, name: str):
            if self.group == "alpasim.services":
                assert name == "video_model"
                return FakeRendererServiceClass
            raise AssertionError(f"Unexpected registry group: {self.group}")
    ...
    job = AssignedRolloutJob(
        ...
        endpoints=ServiceEndpoints(
            driver=ServiceAddress("localhost:10001", skip=False),
            sensorsim=ServiceAddress("localhost:10002", skip=True),
    ...
    assert result.success is True
    assert captured["raw_config"] == {"foo": "bar"}
